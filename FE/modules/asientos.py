...
def render_page(backend_url: str):
    ...
    # Obtener datos de la transacción actual
    transaccion_data = get_transaccion_data(backend_url, current_transaction)

    if transaccion_data:
        st.info(
            f"📋 Trabajando con Transacción ID: **{current_transaction}** | Tipo: **{transaccion_data.get('categoria', 'N/A')}**")
    else:
        st.warning(f"⚠️ No se pudo cargar la transacción {current_transaction}")
        return
    ...
    with st.expander("➕ Crear Nuevo Asiento", expanded=True):
        create_asiento_form(backend_url, current_transaction, accounts, transaccion_data)
...
def get_transaccion_data(backend_url: str, transaction_id: int) -> Optional[Dict]:
    """Obtener datos de una transacción específica"""
    try:
...
def create_asiento_form(backend_url: str, transaction_id: int, accounts: List[Dict], transaccion_data: Dict):
